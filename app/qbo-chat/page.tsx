...
import { 
  QboCompanyInfoTypedPart, 
  QboCompanyInfoResultPart, 
  QboCustomersTypedPart, 
  QboCustomersResultPart,
  QboInvoicesTypedPart,
  QboInvoicesResultPart,
  QboAccountsTypedPart,
  QboAccountsResultPart,
  QboPaymentsTypedPart,
  QboPaymentsResultPart,
  QboProfitAndLossTypedPart,
  QboProfitAndLossResultPart,
  QboSalesTypedPart,
  QboSalesResultPart,
  QboExpensesTypedPart,
  QboExpensesResultPart,
  QboEstimatesTypedPart,
  QboEstimatesResultPart,
  QboPurchasesTypedPart,
  QboPurchasesResultPart,
  QboBillsTypedPart,
  QboBillsResultPart,
  QboVendorsTypedPart,
  QboVendorsResultPart,
  QboItemSalesTypedPart,
  QboItemSalesResultPart,
  QboCustomerSalesTypedPart,
  QboCustomerSalesResultPart,
  QboVendorExpensesTypedPart,
  QboVendorExpensesResultPart,
  QboTaxAgencyTypedPart,
  QboTaxAgencyResultPart,
  QboTaxReportTypedPart,
  QboTaxReportResultPart
} from '@/app/components/fallback/qbo-fallback';
              ...
              for (const m of messages as any[]) {
                ...
                if (parts.length === 0) {
                  ...
                  parts.forEach((part: any, idx: number) => {
                    ...
                    // QBO tool typed parts
                    if (part?.type === 'tool-getQboCompanyInfo' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboCompanyInfo-${idx}`, m.role, (
                        <QboCompanyInfoTypedPart state={part.state} input={part.input} id={`${m.id}-getQboCompanyInfo-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboCustomers' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboCustomers-${idx}`, m.role, (
                        <QboCustomersTypedPart state={part.state} input={part.input} id={`${m.id}-getQboCustomers-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboInvoices' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboInvoices-${idx}`, m.role, (
                        <QboInvoicesTypedPart state={part.state} input={part.input} id={`${m.id}-getQboInvoices-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboAccounts' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboAccounts-${idx}`, m.role, (
                        <QboAccountsTypedPart state={part.state} input={part.input} id={`${m.id}-getQboAccounts-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboPayments' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboPayments-${idx}`, m.role, (
                        <QboPaymentsTypedPart state={part.state} input={part.input} id={`${m.id}-getQboPayments-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboProfitAndLoss' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboProfitAndLoss-${idx}`, m.role, (
                        <QboProfitAndLossTypedPart state={part.state} input={part.input} id={`${m.id}-getQboProfitAndLoss-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboSales' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboSales-${idx}`, m.role, (
                        <QboSalesTypedPart state={part.state} input={part.input} id={`${m.id}-getQboSales-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboExpenses' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboExpenses-${idx}`, m.role, (
                        <QboExpensesTypedPart state={part.state} input={part.input} id={`${m.id}-getQboExpenses-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboEstimates' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboEstimates-${idx}`, m.role, (
                        <QboEstimatesTypedPart state={part.state} input={part.input} id={`${m.id}-getQboEstimates-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboPurchases' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboPurchases-${idx}`, m.role, (
                        <QboPurchasesTypedPart state={part.state} input={part.input} id={`${m.id}-getQboPurchases-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboBills' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboBills-${idx}`, m.role, (
                        <QboBillsTypedPart state={part.state} input={part.input} id={`${m.id}-getQboBills-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboVendors' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboVendors-${idx}`, m.role, (
                        <QboVendorsTypedPart state={part.state} input={part.input} id={`${m.id}-getQboVendors-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboItemSales' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboItemSales-${idx}`, m.role, (
                        <QboItemSalesTypedPart state={part.state} input={part.input} id={`${m.id}-getQboItemSales-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboCustomerSales' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboCustomerSales-${idx}`, m.role, (
                        <QboCustomerSalesTypedPart state={part.state} input={part.input} id={`${m.id}-getQboCustomerSales-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboVendorExpenses' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboVendorExpenses-${idx}`, m.role, (
                        <QboVendorExpensesTypedPart state={part.state} input={part.input} id={`${m.id}-getQboVendorExpenses-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboTaxAgency' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboTaxAgency-${idx}`, m.role, (
                        <QboTaxAgencyTypedPart state={part.state} input={part.input} id={`${m.id}-getQboTaxAgency-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboTaxReport' && part.state !== 'output-available') {
                      pushTool(`${m.id}-getQboTaxReport-${idx}`, m.role, (
                        <QboTaxReportTypedPart state={part.state} input={part.input} id={`${m.id}-getQboTaxReport-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    // QBO tool result parts
                    if (part?.type === 'tool-getQboCompanyInfo' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboCompanyInfo-${idx}`, m.role, (
                        <QboCompanyInfoResultPart output={part.output} id={`${m.id}-tool-result-getQboCompanyInfo-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboCustomers' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboCustomers-${idx}`, m.role, (
                        <QboCustomersResultPart output={part.output} id={`${m.id}-tool-result-getQboCustomers-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboInvoices' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboInvoices-${idx}`, m.role, (
                        <QboInvoicesResultPart output={part.output} id={`${m.id}-tool-result-getQboInvoices-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboAccounts' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboAccounts-${idx}`, m.role, (
                        <QboAccountsResultPart output={part.output} id={`${m.id}-tool-result-getQboAccounts-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboPayments' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboPayments-${idx}`, m.role, (
                        <QboPaymentsResultPart output={part.output} id={`${m.id}-tool-result-getQboPayments-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboProfitAndLoss' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboProfitAndLoss-${idx}`, m.role, (
                        <QboProfitAndLossResultPart output={part.output} id={`${m.id}-tool-result-getQboProfitAndLoss-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboSales' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboSales-${idx}`, m.role, (
                        <QboSalesResultPart output={part.output} id={`${m.id}-tool-result-getQboSales-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboExpenses' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboExpenses-${idx}`, m.role, (
                        <QboExpensesResultPart output={part.output} id={`${m.id}-tool-result-getQboExpenses-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboEstimates' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboEstimates-${idx}`, m.role, (
                        <QboEstimatesResultPart output={part.output} id={`${m.id}-tool-result-getQboEstimates-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboPurchases' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboPurchases-${idx}`, m.role, (
                        <QboPurchasesResultPart output={part.output} id={`${m.id}-tool-result-getQboPurchases-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboBills' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboBills-${idx}`, m.role, (
                        <QboBillsResultPart output={part.output} id={`${m.id}-tool-result-getQboBills-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboVendors' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboVendors-${idx}`, m.role, (
                        <QboVendorsResultPart output={part.output} id={`${m.id}-tool-result-getQboVendors-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboItemSales' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboItemSales-${idx}`, m.role, (
                        <QboItemSalesResultPart output={part.output} id={`${m.id}-tool-result-getQboItemSales-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboCustomerSales' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboCustomerSales-${idx}`, m.role, (
                        <QboCustomerSalesResultPart output={part.output} id={`${m.id}-tool-result-getQboCustomerSales-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboVendorExpenses' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboVendorExpenses-${idx}`, m.role, (
                        <QboVendorExpensesResultPart output={part.output} id={`${m.id}-tool-result-getQboVendorExpenses-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboTaxAgency' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboTaxAgency-${idx}`, m.role, (
                        <QboTaxAgencyResultPart output={part.output} id={`${m.id}-tool-result-getQboTaxAgency-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }

                    if (part?.type === 'tool-getQboTaxReport' && part.state === 'output-available') {
                      pushTool(`${m.id}-tool-result-getQboTaxReport-${idx}`, m.role, (
                        <QboTaxReportResultPart output={part.output} id={`${m.id}-tool-result-getQboTaxReport-${idx}`} idx={idx} role={m.role} />
                      ));
                      return;
                    }
